...
//The people object provides methods and events to manange
//a collection of person objects. Its public methods includ:
//  * get_user() - reutrn the current user person object.
//    If the current user is not signed-in, an anonymous person object
//    is returned
//  * get_db() - return the TaffyDB database of all the person
//    objects - including the current user - pre-scorted.
//  * login( <user_name> ) - login as the user with the provied
//    user name. The current user object is changed to reflect
//  * logout() - revert the current object to anonymous.
//
...
//Person objects is represented by a person object.
...
//  * cid - string client id. This is alwaus defined, and
//    is only different form the id attribute
//    if the client data is not synced with the bacnend.
...
spa.model = (function() {
    ...
    removePerson = function( person ) {
        ...
        if( people.id === configMap.anon_id ) {
            return false;
        }
    ...
    //The chat object API
    //-------------------
    //The chat object is available at the spa.model.chat/
    ...
    //    is chating. If there is no chatee, null is returned.
    //  * set_chatee() - set the chatee to the person
    ...
    //    It publishes a 'spa-updatechatee' global custom event.
    ...
    //    A subscriber to this event shoud get the people_db
    //    form the people model for the updated data.
    //  * spa-updatechat - This is published when a new message
    //    is received or sent. A ,ap of the form:
    //      { dest_id: <chatee_id>,
